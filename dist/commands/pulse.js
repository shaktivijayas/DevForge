...
function buildDashboard(user, repos, recent, totalStars) {
    ...
    // ── Left panel — Profile ──────────────────────────────────────
    const profileBox = grid.set(1, 0, 3, 5, blessed_1.default.box, {
        label: ' 👤 Profile ',
        border: { type: 'line' },
        style: { label: { fg: 'cyan' }, border: { fg: 'cyan' } },
        content: `  Name:     ${user.name || user.login}\n` +
            `  Bio:      ${user.bio ? padRight(user.bio, 38) : 'N/A'}\n` +
            `  Location: ${user.location || 'N/A'}`,
        padding: { left: 1, right: 1 },
    });
    // ── Left panel — Stats ────────────────────────────────────────
    const statsBox = grid.set(4, 0, 3, 5, blessed_1.default.box, {
        label: ' 📊 Stats ',
        border: { type: 'line' },
        style: { label: { fg: 'yellow' }, border: { fg: 'yellow' } },
        content: `  Followers:   ${user.followers.toLocaleString()}\n` +
    ...
    const topList = grid.set(7, 0, 5, 5, blessed_1.default.list, {
    ...
    const repoTable = grid.set(1, 5, 6, 7, blessed_contrib_1.default.table, {
    ...
    const barChart = grid.set(7, 5, 5, 7, blessed_contrib_1.default.bar, {
    ...
    screen.key(['r', 'R'], async () => {
        ...
        try {
            const [freshUser, freshRepos] = await Promise.all([
                (0, github_1.fetchUser)(user.login),
                (0, github_1.fetchRepos)(user.login),
            ]);
            const freshRecent = (0, github_1.getRecentRepos)(freshRepos, 5);
            const freshStars = (0, github_1.calcTotalStars)(freshRepos);
            profileBox.setContent(`  Name:     ${freshUser.name || freshUser.login}\n` +
                `  Bio:      ${freshUser.bio ? padRight(freshUser.bio, 38) : 'N/A'}\n` +
                `  Location: ${freshUser.location || 'N/A'}`);
            statsBox.setContent(`  Followers:   ${freshUser.followers.toLocaleString()}\n` +
            ...
            bottomBar.setContent('  Press Q to quit | R to refresh | Last updated: ' + new Date().toLocaleTimeString());
            screen.render();
        }
        catch (e) {
...
async function runPulse() {
    (0, display_1.showSmallHeader)('GitPulse — GitHub Dashboard');
    try {
        let username = (0, storage_1.getGithubUsername)();
        if (!username) {
            const answer = await inquirer_1.default.prompt([
                {
                    type: 'input',
                    name: 'username',
                    message: chalk_1.default.cyan('Enter your GitHub username:'),
                    validate: (v) => v.trim().length > 0 || 'Username cannot be empty',
                },
            ]);
            username = answer.username.trim();
            (0, storage_1.setGithubUsername)(username);
        }
        const spinner = (0, display_1.createOraSpinner)(`Fetching data for @${username}...`);
        spinner.start();
        const [user, repos] = await Promise.all([(0, github_1.fetchUser)(username), (0, github_1.fetchRepos)(username)]);
        const recent = (0, github_1.getRecentRepos)(repos, 5);
        const totalStars = (0, github_1.calcTotalStars)(repos);
        spinner.succeed(chalk_1.default.green(`Loaded profile for @${username}`));
        buildDashboard(user, repos, recent, totalStars);
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        (0, display_1.errorBox)(`GitPulse error: ${msg}`);
        process.exit(1);
    }
}
